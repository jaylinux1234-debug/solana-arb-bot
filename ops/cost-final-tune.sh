...
echo "=== Final Cost Reduction for First Fills ==="

sed -i 's/V2_BASE_COST_BPS=.*/V2_BASE_COST_BPS=4.5/' .env
sed -i 's/V2_SLIPPAGE_BUFFER_BPS=.*/V2_SLIPPAGE_BUFFER_BPS=1.8/' .env
# Integer only — 5.0 crashes Pydantic
sed -i 's/CEX_DEX_MIN_GROSS_SPREAD_BPS=.*/CEX_DEX_MIN_GROSS_SPREAD_BPS=5/' .env
sed -i 's/V2_ROUNDTRIP_SOFT_PASS_FACTOR=.*/V2_ROUNDTRIP_SOFT_PASS_FACTOR=0.89/' .env
...
docker ps -aq --filter name=solana-arb-monitor | xargs -r docker rm -f
bash scripts/restart-monitor.sh

echo "✅ Final cost tune applied"
