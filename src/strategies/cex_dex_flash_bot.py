...
from src.execution.jito import configure_jito, send_jito_bundle
from src.strategies.cex_dex_core import analyze_cex_dex_spread, cex_dex_ai_min_confidence
from src.utils.ai import ai_agent_decide

try:
    from jito_searcher_client import JitoClient
except ImportError:
    JitoClient = None
...
async def execute_flash_loan_opportunity(
    ...
    *,
    ...
    """Simulate first, then send through Jito searcher client or internal bundle relay."""

    sim = await client.simulate_transaction(tx)
    if sim.value.err is not None:
        logger.warning("Simulation failed, skipping bundle: %s", sim.value.err)
        return None

    logger.info("Simulation successful, attempting bundle submission")

    if JitoClient is not None:
        try:
            searcher_client = JitoClient(
                block_engine_url=os.getenv(
                    "JITO_BLOCK_ENGINE_URL",
                    "https://ny.mainnet.block-engine.jito.wtf",
                ),
                tip_lamports=int(os.getenv("JITO_TIP_LAMPORTS", str(tip_lamports or 100_000))),
            )
            bundle = await searcher_client.build_bundle([tx])
            result = await searcher_client.send_bundle(bundle)
            bundle_id = getattr(result, "bundle_id", None) or getattr(result, "value", None)
            if bundle_id:
                logger.info("Jito searcher client bundle sent: %s", bundle_id)
                return str(bundle_id)
            logger.warning("Jito searcher client returned no bundle id")
        except Exception as exc:
            logger.warning(
                "Jito searcher client bundle send failed, falling back to internal relay: %s",
                exc,
            )
    ...
    bundle_id = await send_jito_bundle(
        ...
        client=client,
        keypair=keypair,
        tip_lamports=tip_lamports or int(os.getenv("JITO_TIP_LAMPORTS", "100000")),
    )
    if bundle_id:
        logger.info("Internal Jito bundle sent: %s", bundle_id)
    return bundle_id
...
async def get_cex_price():
    """SOL/USDC mid via shared feed (Backpack → Bybit → OKX → KuCoin)."""
    try:
        return await cex_feed.get_price("SOL/USDC")
...
async def execute_flash_arb(cex_price: float, dex_price: float):
    ...
    if not analysis:
        logger.info("❌ Invalid CEX/DEX mids")
        return

    spread_pct = analysis.spread_pct_signed
    ...
    logger.info(
        "CEX-DEX Flash Arb | Direction=%s | Spread=%+.3f%%",
        direction,
        spread_pct,
    )

    if circuit_breaker.should_pause():
        logger.info("CEX-DEX flash bot: paused (circuit breaker)")
        return
    ...
    if abs(spread_pct) < MIN_SPREAD_PCT:
        logger.info(f"❌ Spread too small (< {MIN_SPREAD_PCT}%)")
        await client.close()
        return

    # Calculate SOL to sell (convert USDC micro → SOL lamports)
    ...
    decision = await ai_agent_decide(
        opportunity, balance, min_confidence=cex_dex_ai_min_confidence()
    )
    ...
    if decision.get("final_action") != "APPROVE":
        logger.info(f"🛑 AI REJECTED: {(decision.get('trade_decision') or {}).get('reasoning')}")
        print("🛑 AI REJECTED")
        await client.close()
        return
    ...
    try:
        ...
        if TEST_MODE:
            ...
            if ok:
                bundle_id = await execute_flash_loan_opportunity(
                    tx,
                    client,
                    keypair,
                    tip_lamports=85000,
